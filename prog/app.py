...
cities_list = ["Berlin","Лондон","Київ","Прага","Варшава","Tokio","Дніпро","Запоріжжя","Вінниця"]
...
class City_object(ctk.CTkFrame):
    def __init__(self,master,width,height,fg_color,corner_radius,x,y,city_name):
        ...
        self.city_label = ctk.CTkLabel(master=self, width=width/3, height=height/4,font=("Roboto Slab",height/3),text_color="#FFFFFF",text=city_name)
...
def big_screen(master):
    ...
    def search_command(event):
        ...
        weather_request()
        weather_request2()
    search = ctk.CTkEntry(master = app, width = 150, height = 50, corner_radius= 28, fg_color= "#096C82", placeholder_text= "Пошук", text_color= "#FFFFFF", border_color= "#FFFFFF")
    search.place(x = width // 1.3, y = height // 40)
    search.bind("<Return>", search_command)

    title = ctk.CTkLabel(master = app, width= 100, height= 55, text= "Поточна позиція", font=("Roboto Slab",45),text_color="#FFFFFF")
    title.place(x= width // 2, y = height // 2.5 - 100, anchor= ctk.CENTER)

    weather_status = ctk.CTkLabel(master=app,width=100,height=25,text="",font=("Roboto Slab",25),text_color="#FFFFFF")
    weather_status.place(x= width // 2, y = height // 2.5 + 50, anchor= ctk.CENTER)
    ...
    temperature.place(x= width // 2, y = height // 2.5, anchor= ctk.CENTER)
    
    city_name = ctk.CTkLabel(master=app,width=100,height=30,text="",font=("Roboto Slab",30),text_color="#FFFFFF")
    city_name.place(x= width // 2, y = height // 2.5 - 50, anchor= ctk.CENTER)
    ...
    for city in cities_list:
        cities.append(City_object(master= scroll_frame, width= 250, height= 75, fg_color= "#096C82", corner_radius= 28, x = y, y = x, city_name= city))
        y += 1
